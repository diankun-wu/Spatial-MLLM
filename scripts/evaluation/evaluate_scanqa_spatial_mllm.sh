...
for nframe in "${nframes[@]}"; do
    ...
    # --- run experiment ---
    python src/evaluation/scanqa/eval_scanqa.py \
        --model_path "$MODEL_PATH" \
        --model_type "$MODEL_TYPE" \
        --nframes $nframe \
        --annotation_path "datasets/evaluation/scanqa/scanqa_val_llava_style.json" \
        --video_folder "datasets/visuals/scannet/videos" \
        --batch_size 1 \
        --output_dir "$EXP_DIR" \
